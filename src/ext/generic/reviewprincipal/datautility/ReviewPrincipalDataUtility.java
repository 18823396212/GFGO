package ext.generic.reviewprincipal.datautility;
 ...
 *
 */
public class ReviewPrincipalDataUtility extends AbstractDataUtility{

	 private static final Logger LOGGER = LogR.getLogger(ReviewPrincipalDataUtility.class.getName());
	...
	public Object getDataValue(String s, Object obj, ModelContext modelcontext) throws WTException{
		...
		if (obj instanceof WTUser){
			...
			if (s.equals("extlistGroupFullName")||"genericRoleParticipantsName".equals(s)
					||"name".equals(s)){
				...
				//WTContainer container = commandBean.getPageOid().getContainer();				
			...
			//	Role role = Role.toRole(((NmSimpleOid)obj).getInternalName());
                ...
                for(int i=0;i<augmentActs.length;i++){
                	...
                	String augmentActivity=augmentActs[i];
                	...
                	if(augmentActivity.contains(ReviewConstant.REQUIRED_ACT_SEP)){
                		...
                		for(int j=0;j<activity.length;j++){
                			...
                			if(activity[j].equals(activName.trim())){
				...
				ReviewActivityUtil activityUtil=new ReviewActivityUtil();
				
				Persistable pbo = workItem.getPrimaryBusinessObject().getObject();

				WfProcess wfprocess = wfactivity.getParentProcess();
				
				String sheetName=activityUtil.getSheetName((WTObject)pbo);
				
				RowBean rowBean=activityUtil.getRowBean(null, wfprocess, (WTObject)pbo, sheetName);
				
				CellBean cellBean = activityUtil.getCellBean((WTObject)pbo, null, wfprocess, activName);
				...
				if(cellBean != null){
					...
					if(users.size()>0){
						...
						sbf.append(WTMessage.getLocalizedMessage(resource, "PERSON", null, locale)+"：[");
...
//							sbf.append(role.getDisplay(locale)).append("(").append(role.toString()).append(")  ");
