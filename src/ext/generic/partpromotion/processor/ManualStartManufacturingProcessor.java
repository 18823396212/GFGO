package ext.generic.partpromotion.processor;
 ...
 * TODO
 */
public class ManualStartManufacturingProcessor extends DefaultObjectFormProcessor {
	private static String CLASSNAME = ManualStartManufacturingProcessor.class.getName();
	...
	public FormResult doOperation(NmCommandBean nmCommandBean, List<ObjectBean> arg1)
		...
		try{
			...
			if(workFlowBase.startWorkFlow(part, workFlowName)){
				String msg = WTMessage.getLocalizedMessage(RESOURCE, "START_MANUALPARTSIGN_MSG", null, locale);
				feedbackmessage = new FeedbackMessage(FeedbackType.SUCCESS, null,msg, null, new String[] {});
			}
		}
		catch(Exception e){
			e.printStackTrace();
			String error = WTMessage.getLocalizedMessage(RESOURCE, "START_MANUALPARTSIGN_ERR", null, locale);
