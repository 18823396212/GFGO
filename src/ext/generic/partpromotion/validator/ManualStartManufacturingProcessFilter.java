 ...
 * 手动启动流程入口
 */
public class ManualStartManufacturingProcessFilter extends DefaultSimpleValidationFilter {
	private static String CLASSNAME = ManualStartManufacturingProcessFilter.class.getName();
	...
	public UIValidationStatus preValidateAction(UIValidationKey uivalidationkey, UIValidationCriteria uivalidationcriteria) {
		LOGGER.debug(" ===ManualStartManufacturingProcessValidator===" );
...
//							uivalidationstatus = UIValidationStatus.DISABLED;
	...
	private static boolean showManualStartProcess(WTPart part) throws WTException{
		...
		PartPromotionXMLConfigUtil partPromotionXMLConfigUtil = new PartPromotionXMLConfigUtil("partManufacturerRule.xml");
		...
		//判断是否为Design视图，对于Design视图不处理 
		if(partView.equals("Design")){
			LOGGER.debug( "Object is Design View, return false ");
			return false;
		}
