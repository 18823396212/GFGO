    ...
    public static boolean checkLibrary(WTObject obj) {
        ...
        try {
            if (obj instanceof WTPart) {
                ...
                //modify by 20180605
                if (CISBusinessRuleXML.getInstance().getLibrary().contains(name) && CISBusinessRuleXML.getInstance().getCls().contains(clsname)) {
                    result = true;
                }
            }
        } catch (Exception e) {
        ...
        try {
            ...
            if (al != null && al.size() > 0) { // 判断是否有随签对象
                ...
                for (int i = 0; i < al.size(); i++) {
                    ...
                    if (obj instanceof WTPart) {
                        WTPart part = (WTPart) obj;
                        if (checkLibrary(part)) publishData(part, connection);
                    } else if (obj instanceof WTDocument) {
                        WTDocument doc = (WTDocument) obj;
                        ...
                        if (qrdoc != null) {
                            while (qrdoc.hasMoreElements()) {
                                ...
                                if (typeName.equals("symbol") || typeName.equals("footprint")) {
                                    ...
                                    if (ci == null) {
                                        sb.append(part.getDisplayIdentifier() + " 关联的文档 " + doc.getDisplayIdentifier() + " 无主内容,不能完成任务!");
                                    } else {
                                        if (!refVersion.equals("A")) {// 说明是变更
                                            ...
                                            if (!befad.getFileName().equals(ad.getFileName())) {
                                                sb.append(" 文档对象 " + doc.getDisplayIdentifier() + " 和上一最新版本对象" + befortDoc.getDisplayIdentifier() + "的主内容名称不一致!");
                                            }
                                        }
                                    }

                                } else {
                                    sb.append("部件对象" + part.getDisplayIdentifier() + "关联的文档对象" + doc.getDisplayIdentifier() + " 不是symbol 或 footprint 文档类型");
                                }
                            }
                        } else {
