...
public class SourceMappingUtil {
	private static final String clazz = SourceMappingUtil.class.getName() ;
	private static final Logger logger = LogR.getLogger(clazz);
	private static Map<String,String> sourceMap = null ;

	private static final String SOURCE_EXCEL_FILE = "SourceMapping.xls" ;
	
	private static final int PDM_SOURCE_COL_INDEX = 0 ;
	private static final int ERP_SOURCE_COL_INDEX = 1 ;
	
	static{
		//配置文件与ext.generic.integration.erp.config.UnitMappingUtil.class文件在同一路径下
		String filePath = SourceMappingUtil.class.getResource("").getPath() ;
		...
		sourceMap = parseExcel( filePath + SOURCE_EXCEL_FILE ) ;
	}
	 ...
	 * @return
	 */
	public static String getERPSource( String pdmUnit){
		String erpUnit = "" ;
	...
	private static Map<String,String> parseExcel(String file){
		...
		if(sheet != null){
			...
			while( ite.hasNext() ){
				...
				if( row != null ){
					String pdmUnit = ExcelUtil.getCellValueByIndex(row, PDM_SOURCE_COL_INDEX, false);
					String erpUnit = ExcelUtil.getCellValueByIndex(row, ERP_SOURCE_COL_INDEX, true);
