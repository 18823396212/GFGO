...
public class UnitMappingUtil {
	private static final String clazz = UnitMappingUtil.class.getName() ;
	...
	private static final int ERP_UNIT_COL_INDEX = 1 ;
	
	static{
		//配置文件与ext.generic.integration.erp.config.UnitMappingUtil.class文件在同一路径下
		String filePath = UnitMappingUtil.class.getResource("").getPath() ;
		...
		unitMap = parseExcel( filePath + UNIT_EXCEL_FILE ) ;
	}
	 ...
	 * @return
	 */
	public static String getERPUnit( String pdmUnit){
		String erpUnit = "" ;
		...
		if(sheet != null){
			...
			while( ite.hasNext() ){
				...
				if( row != null ){
					...
					String erpUnit = ExcelUtil.getCellValueByIndex(row, ERP_UNIT_COL_INDEX, false);
