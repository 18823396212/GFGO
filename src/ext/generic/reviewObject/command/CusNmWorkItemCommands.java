 ...
 * 修   改  人：毛兵义
 * 关联活动：
 * 修改内容：标准属性变更流程增加物料废弃的功能，保存变更类型的值
 * </pre>
 */
...
public class CusNmWorkItemCommands extends NmWorkItemCommands implements Externalizable, ModifyConstants, ChangeConstants {
	...
	private static void checkUnPassComments(NmCommandBean cb) throws WTException {
		...
		while (parameterNames.hasMoreElements()) { // loop through all the
													// user's form fields
			...
			if (key.indexOf(ROUTER_EVENT) >= 0 && key.lastIndexOf("old") == -1) {
				...
				if (key.indexOf(ROUTER_CHECK) >= 0) {
					eventValue = key.substring(key.indexOf(ROUTER_CHECK) + NmWorkItemCommands.ROUTER_CHECK.length(),
							key.lastIndexOf("___"));
				} else {
				...
				for (String noEmptyRout : noEmptyRouts) {
					if (eventName.startsWith(noEmptyRout)) {
						if (StringUtils.isBlank(comments)) {
							...
							throw new WTException(WTMessage.getLocalizedMessage(RESOURCE, "CHECK_REMARK_NULL", null,
	...
	private static List<String> readWorkflowNoEmptyRoutsFromPreference() {
		...
		try {
			// 读取首选项
			Object preferenceValue = PreferenceHelper.service.getValue("/pisxcustomization/workflowNoEmptyRouts",
	...
	private static FormResult completeOrSave(NmCommandBean nmCommandBean, FormResult formResult) throws WTException {
		...
		if (persistable instanceof WorkItem) {
			...
			if (workFlowNameXML.equals("APPO_PartAttrChangeWF")
				...
				JSONObject attrChangeType = new JSONObject();
				QueryResult queryresult = ProcessReviewObjectLinkHelper.service
						.getProcessReviewObjectLinkByRoleA(wfprocess);
				if (queryresult != null) {
					while (queryresult.hasMoreElements()) {
						// Object getValue=
						// wfprocess.getContext().getValueObject("primaryBusinessObject");
						...
						if (obj instanceof WTPart) {
							...
							// String version =
							// part.getVersionInfo().getIdentifier().getValue()+"."+part.getIterationInfo().getIdentifier().getValue();
							String changeReasonValue = String.valueOf(textArea.get(crStr)) == null ? ""
									: String.valueOf(textArea.get(crStr));
							String preChangeContentValue = String.valueOf(textArea.get(preStr)) == null ? ""
									: String.valueOf(textArea.get(preStr));
							String postChangeContentValue = String.valueOf(textArea.get(postStr)) == null ? ""
									: String.valueOf(textArea.get(postStr));
							String attrChangeTypeValue = String.valueOf(conboBoxMap.get(attrChangeTypeStr)) == null ? ""
									: String.valueOf(conboBoxMap.get(attrChangeTypeStr));
							...
							try {
								...
								attrChangeType.put(branchIdentifier, attrChangeTypeValue);
				...
				context.setValue("attrChangeType", attrChangeType.toString());
				wfprocess.setContext(context);
				PersistenceHelper.manager.save(wfprocess);
			}

			if (workFlowNameXML.equals("APPO_ProductPhaseStateWF")
					&& (wfactivityname.equals("编制") || wfactivityname.equals("修改"))) {
				...
				while (queryresult.hasMoreElements()) {
					...
					if (signedOpinionList.keySet().contains(wfactivityname)) {
						...
						if (opinionValue != null) {
							// QueryResult qr =
							// SignedOpinionHelper.service.queryOpinionValue(wfprocess,obj,wfactivityname,tempUserName);
							QueryResult qr = SignedOpinionHelper.service.queryOpinionValue(wfprocess, obj,
									wfactivityname);
							if (!compareSignedOpinion(opinionValue, qr)) {
								...
								try {
									SignedOpinionHelper.service.newSignedOpinionLink(wfactivityname, opinionValue,
											tempUserName, timestamp, wfprocess, obj);
	...
	public static boolean compareSignedOpinion(String opinionValue, QueryResult qr) {
		...
		if (qr != null && qr.hasMoreElements()) {
			...
			// if(opinionValue.trim().equals(oldOpinion.trim()) ||
			// (tempUserName.trim() + ":" +
			// opinionValue.trim()).equals(oldOpinion.trim() ) ||
			// opinionValue.trim().equals(tempUserName.trim() + ":" +
			// oldOpinion.trim())){
			...
			if (signedOpinionList != null && signedOpinionList.size() > 0) {
				if (signedOpinionList.keySet().contains(wfactivityname)) {
					...
					while (queryresult != null && queryresult.hasMoreElements()) {
						...
						LOGGER.debug("tempKey : = " + tempKey + " wtObjectoid : = " + wtObjectoid
								+ " wtObjectRfoid : = " + wtObjectRfoid + " userName : = " + userName);
						DataUtilityHelper dataUtilityHelper = new DataUtilityHelper(nmCommandBean, wfprocess);
						if (!(dataUtilityHelper.isSign(wfassignedactivity.getName(),
								dataUtilityHelper.isSignHashtable(wfprocess)))) {
							String opinionValue = (String) hashmap.get(tempKey);
							LOGGER.debug("opinionValue : = " + opinionValue);
							if (opinionValue == null || opinionValue.isEmpty()) {
								// throw new
								// wt.util.WTException("有对象没有填写意见，请检查。");
								throw new WTException(WTMessage.getLocalizedMessage(RESOURCE, "SIGN_OPINION_NULL", null,
										SessionHelper.getLocale()));
							}
							...
							if (userName != null && userName.equals(tempUserName)
									|| (oldAssigneeList.contains(userName))) {
								String opinionValue = (String) hashmap.get(tempKey);
								LOGGER.debug("opinionValue : = " + opinionValue);
								if (opinionValue == null || opinionValue.isEmpty()) {
									// throw new
									// wt.util.WTException("有对象没有填写意见，请检查。");
									throw new WTException(WTMessage.getLocalizedMessage(RESOURCE, "SIGN_OPINION_NULL",
	...
	private static List<String> readWorkflowOneRejectFromPreference() {
		...
		try {
			// 读取首选项
			Object preferenceValue = PreferenceHelper.service.getValue("/pisxcustomization/workflowOneRejects",
					PreferenceClient.WINDCHILL_CLIENT_NAME);
