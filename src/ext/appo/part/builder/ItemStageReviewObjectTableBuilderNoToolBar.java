 ...
 * @author Yzhang
 */
@ComponentBuilder("ext.appo.part.builder.ItemStageReviewObjectTableBuilderNoToolBar")
public class ItemStageReviewObjectTableBuilderNoToolBar extends AbstractComponentBuilder {
	private ClientMessageSource msgSource = getMessageSource(
			"ext.generic.reviewObject.resource.ReviewObjectResourceRB");
	private static final Logger LOGGER = LogR.getLogger(ItemStageReviewObjectTableBuilderNoToolBar.class.getName());
	...
	static {
		try {
			...
			String excelPath = codebalocation + sp + "ext" + sp + "generic" + sp + "reviewObject" + sp + "config" + sp
					+ "itemReviewObjectColConfig.xlsx";
		...
		if (wfprocess != null) {
			...
			while (queryresult != null && queryresult.hasMoreElements()) {
				...
				if (!(dataUtilityHelper.isSign(wfassignedactivity.getName(),
						dataUtilityHelper.isSignHashtable(wfprocess))) || userName == null || userName.isEmpty()) {
					reviewObjList.add(obj);
				} else {
					if (userName.equals(principal.getName() + "_" + ((WTUser) principal).getFullName())
