...
public class ChangeTaskDataUtility extends AbstractDataUtility{
	...
	public Object getDataValue(String paramString, Object paramObject, ModelContext paramModelContext) throws WTException {
		...
		if(bool){
			if(paramString.equalsIgnoreCase(ChangeConstants.NEEDDATE_COMPID)){
				...
				PickerInputComponent userPicker = (PickerInputComponent)userPickerDU.createPickerComponent(paramString, paramObject, paramModelContext, bool);
				return userPicker ;
			}else {
	...
	public TextDisplayComponent generateTextDisplayComponent(ModelContext paramModelContext, Object paramObject,
			String keyStr) throws WTException{
		...
		try {
			...
			if(keyStr.equalsIgnoreCase(ChangeConstants.NEEDDATE_COMPID)){
				...
				if(value.contains("-")){
					value = (new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_02)).format(
								(new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_03)).parse(value)) ;
				}
				gui.setValue(value);
			}else{
	...
	public DateInputComponent generateDateInputComponent(ModelContext paramModelContext, Object paramObject, String keyStr) throws WTException{
		...
		try {
			...
			if(PIStringUtils.isNotNull(value)){
				...
				component.setValue(Timestamp.valueOf((new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT)).format(
						(new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_02)).parse((String)value))));
			}else{
