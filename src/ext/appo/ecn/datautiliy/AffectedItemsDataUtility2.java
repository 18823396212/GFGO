...
import wt.change2.RelatedProductData;
...
public class AffectedItemsDataUtility2 extends ChangeLinkAttributeDataUtility {
	...
	public Object getDataValue(String paramString, Object paramObject, ModelContext paramModelContext)
		...
		if (bool) {
			...
			try {
				if (paramString.contains(ChangeConstants.ARTICLEINVENTORY_COMPID)
						|| paramString.contains(ChangeConstants.CENTRALWAREHOUSEINVENTORY_COMPID)
					...
					if (paramObject instanceof WTPart) {
						...
						gui_array.addGUIComponent(generateTextDisplayComponent(paramModelContext, paramObject,
								paramString, getValue(paramModelContext, paramObject, bool, paramString)));
						return gui_array;
					}
				}
			} catch (Exception e) {
		...
		try {
			...
			if (bool) {
				for (NmOid nmOid : withArray) {
					...
					if (object instanceof WTChangeActivity2) {
						...
						if (paramString.contains(ChangeConstants.ARTICLEDISPOSE_COMPID)
								|| paramString.contains(ChangeConstants.INVENTORYDISPOSE_COMPID)
								|| paramString.contains(ChangeConstants.PASSAGEDISPOSE_COMPID)
								|| paramString.contains(ChangeConstants.PRODUCTDISPOSE_COMPID)
								|| paramString.contains(ChangeConstants.CHANGETYPE_COMPID)) {
							if (paramObject instanceof WTPart) {
								GUIComponentArray gui_array = new GUIComponentArray();
								gui_array.addGUIComponent(generateComboBox(paramModelContext, paramObject, paramString,
										getValue(paramModelContext, paramObject, bool, paramString)));
								return gui_array;
							}
						} else if (paramString.contains(ChangeConstants.COMPLETIONTIME_COMPID)) {
							GUIComponentArray gui_array = new GUIComponentArray();
							gui_array.addGUIComponent(generateDateInputComponent(paramModelContext, paramObject,
									paramString, getValue(paramModelContext, paramObject, bool, paramString)));
							return gui_array;
			...
			gui_array.addGUIComponent(generateTextDisplayComponent(paramModelContext, paramObject, paramString,
					getValue(paramModelContext, paramObject, false, paramString)));
			return gui_array;
		} catch (Exception e) {
			e.printStackTrace();
			throw new WTException(e.getLocalizedMessage());
	 ...
	 * @throws WTException
	 */
	public TextDisplayComponent generateTextDisplayComponent(ModelContext paramModelContext, Object paramObject,
			String keyStr, Object value) throws WTException {
	 ...
	 * @throws WTException
	 */
	public TextBox generateTextBox(ModelContext paramModelContext, Object paramObject, String keyStr, Object value)
			throws WTException {
	 ...
	 * @throws WTException
	 */
	public ComboBox generateComboBox(ModelContext paramModelContext, Object paramObject, String keyStr, Object value)
			throws WTException {
		// 枚举定义
		Map<ArrayList<String>, ArrayList<String>> enumMap = null;
		...
		ArrayList<String> nextArray = enumMap.keySet().iterator().next();
		// 内部名称
		ArrayList<String> keyArray = new ArrayList<String>();
		keyArray.add("");
		keyArray.addAll(nextArray);
		// 显示名称
		ArrayList<String> displayArray = new ArrayList<String>();
		displayArray.add("");
		displayArray.addAll(enumMap.get(nextArray));
		// 选取值
	 ...
	 * @throws WTException
	 */
	public DateInputComponent generateDateInputComponent(ModelContext paramModelContext, Object paramObject,
			String keyStr, Object value) throws WTException {
		...
		try {
			...
			if (value != null) {
				...
				if (valueStr.contains("-")) {
					valueStr = (new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_02))
							.format((new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_03)).parse(valueStr));
				}
				component.setValue(Timestamp.valueOf((new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT))
						.format((new SimpleDateFormat(ChangeConstants.SIMPLE_DATE_FORMAT_02)).parse(valueStr))));
			} else {
	 ...
	 * @throws WTException
	 */
	public Object getValue(ModelContext paramModelContext, Object paramObject, Boolean isCreateEdit, String compid)
			throws WTException {
		try {
			/**** 受影响对象 表单数据处理 ****/
			if (isCreateEdit) {
				if (compid.contains(ChangeConstants.ARTICLEINVENTORY_COMPID)
						|| compid.contains(ChangeConstants.CENTRALWAREHOUSEINVENTORY_COMPID)
						|| compid.contains(ChangeConstants.PASSAGEINVENTORY_COMPID)) {
					// 对象为部件时查询
					if (!(paramObject instanceof WTPart)) {
						return "";
					}
					WTPart part = (WTPart) paramObject;
					List<?> datasList = InventoryPriceIbatis.queryInventoryPrice(part.getNumber(), "total",
							(WTUser) SessionHelper.getPrincipal());
			...
			if (compid.contains("childNumber")) {
				...
				return value;
			} else if (compid.equals("ggms") || compid.contains("sscpx") || compid.contains("ssxm")
					|| compid.contains("xsxh") || compid.contains("cpzt") || compid.contains("nbxh")) {
				if (paramObject instanceof WTPart) {
			...
			if (compid.contains("CollectionNumber")) {
				// if(paramObject instanceof WTPart){
				// String childsNumber = getChildsNumber(paramModelContext,
				// (WTPart)paramObject) ;
				// if(PIStringUtils.isNotNull(childsNumber)){
				// if(childsNumber.contains(ChangeConstants.USER_KEYWORD)){
				// childsNumber = childsNumber.substring(0,
				// childsNumber.indexOf(ChangeConstants.USER_KEYWORD)) ;
				// }
				// }
				// value = childsNumber ;
				// }
				...
				if (paramObject instanceof WTDocument) {
					System.out.println("Start to get linkdes part---->>>>>>>");
					return getlinkPartNumber(paramModelContext, (WTDocument) paramObject);
				}
				return value;
			}
			...
			if (value == null) {
				if (compid.contains(ChangeConstants.AADDESCRIPTION_COMPID)
						|| compid.contains(ChangeConstants.CRDESCRIPTION_COMPID)) {
					...
					if (modelObject instanceof ChangeableObjectBean) {
						...
						if (object instanceof WTChangeOrder2) {
							Collection<ChangeActivityIfc> changeActivities = ChangeUtils
									.getChangeActivities((WTChangeOrder2) object);
							for (ChangeActivityIfc changeActivityIfc : changeActivities) {
								AffectedActivityData affectedActivityData = ChangeUtils
										.getAffectedActivity(changeActivityIfc, (Changeable2) modelObject);
				...
				if (compid.contains(ChangeConstants.ARTICLEDISPOSE_COMPID)
						|| compid.contains(ChangeConstants.INVENTORYDISPOSE_COMPID)
						|| compid.contains(ChangeConstants.PASSAGEDISPOSE_COMPID)
						|| compid.contains(ChangeConstants.PRODUCTDISPOSE_COMPID)
						|| compid.contains(ChangeConstants.CHANGETYPE_COMPID)) {
					if (!isCreateEdit) {
					...
					if (PIStringUtils.isNotNull(datasJSON)) {
						...
						for (int i = 0; i < jsonArray.length(); i++) {
							...
							Persistable persistable = (new ReferenceFactory())
									.getReference(jsonObject.getString(ChangeConstants.OID_COMPID)).getObject();
				...
				if (datasArray != null) {
					...
					for (Map.Entry<Integer, Map<String, String>> entryMap : datasMap.entrySet()) {
						for (Map.Entry<String, String> enumEntryMap : entryMap.getValue().entrySet()) {
							interiorArray.add(
									enumEntryMap.getKey() + ChangeConstants.USER_KEYWORD + enumEntryMap.getValue());
	...
	public String getlinkPartNumber(ModelContext paramModelContext, WTDocument doc) throws WTException {
		StringBuilder returnStr = new StringBuilder();
		if (paramModelContext == null || doc == null) {
			return returnStr.toString();
		}
		QueryResult qr = PartDocServiceCommand.getAssociatedDescParts(doc);
		System.out.println("des doc==" + qr.size());
		// 获取页面添加数据
		Map<Persistable, Map<String, String>> pageChangeTaskArray = ChangeUtils
				.getPageChangeTaskArray(paramModelContext.getNmCommandBean());
		System.out.println("pageChangeTaskArray size===" + pageChangeTaskArray.size());
		System.out.println("pageChangeTaskArray size===" + pageChangeTaskArray);
			...
			while (qr.hasMoreElements()) {
				...
				for (Map.Entry<Persistable, Map<String, String>> entryMap : pageChangeTaskArray.entrySet()) {
					...
					if (persistable instanceof WTPart) {
						...
						System.out.println("persistable Number()===" + ChangeUtils.getNumber(persistable));
						if (ChangeUtils.getNumber(persistable).equals(part.getNumber())) {
							if (returnStr.length() > 0) {
								returnStr.append(ChangeConstants.USER_KEYWORD);
							}
							returnStr.append(part.getNumber());
							break;
						}
					}
				}
				...
				while (qr.hasMoreElements()) {
					...
					for (Changeable2 changeable2 : changeablesBefore) {
						if (changeable2 instanceof WTPart) {
							...
							if (ChangeUtils.getNumber(changeable2).equals(part1.getNumber())) {
								if (returnStr.length() > 0) {
									returnStr.append(ChangeConstants.USER_KEYWORD);
								}
								returnStr.append(part1.getNumber());
								break;
							}
						}
					}
