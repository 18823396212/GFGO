				...
				if (compid.contains(ChangeConstants.ARTICLEINVENTORY_COMPID)
					...
					if (ip.equals("172.32.252.61") || ip.equals("172.32.252.70") || ip.equals("172.32.252.71")) {
						...
						if (rList != null && rList.size() > 0) {
							System.out.println(
									"查询数据库表 EXT_STOCK_TAB下物料==" + part.getNumber() + "==大版本==" + mVersion + "==在制数量=="
											+ rList.get(0) + "==库存数量==" + rList.get(1) + "==在途数量==" + rList.get(2));
							if (compid.contains(ChangeConstants.ARTICLEINVENTORY_COMPID)) {
	...
	public String getChildsNumber(ModelContext paramModelContext, WTPart parentPart) throws WTException {
		...
		if (pageChangeTaskArray != null && pageChangeTaskArray.size() > 0) {
			...
			if (object instanceof WTChangeOrder2) {
				...
				//获取暂存的受影响对象
				Set<Persistable> persistables = ModifyHelper.service.queryPersistable((WTChangeOrder2) object, ModifyConstants.LINKTYPE_1);
				for (Persistable persistable : persistables) {
					if (persistable instanceof WTPart) {
						children:for (WTPart childPart : childArray) {
							if (ChangeUtils.getNumber(persistable).equals(childPart.getNumber())) {
                                String[] childNumbers = returnStr.toString().split(ChangeConstants.USER_KEYWORD);
                                for (int i = 0; i <childNumbers.length ; i++) {
                                    if (childNumbers[i].equals(childPart.getNumber())){
                                        break children;
                                    }
                                }
                                if (returnStr.length() > 0) {
