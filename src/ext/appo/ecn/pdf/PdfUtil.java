...
import ext.appo.change.report.EcnChangeTaskService;
...
//					Persistable persistable = link.getPersistable();
//				}
                Set<Persistable> Persistables = ModifyHelper.service.queryPersistable(ecn, LINKTYPE_2);
...
//            Collection<ChangeTaskBean> generateChangeTaskBeans;
            Collection<ChangeTaskInfoBean> changeTaskInfoBeans;
//            if ("APPO_ECNWF".equals(templateName)) {
            changeTaskInfoBeans = getChangeTaskBeans(ecn);
            if (!changeTaskInfoBeans.isEmpty()) {
                List<Map<String, String>> list = new ArrayList<>();
                for (ChangeTaskInfoBean changeTaskInfoBean : changeTaskInfoBeans) {
                    //无任务主题不显示（新增的）
                    if (changeTaskInfoBean.getTaskTheme() != null && changeTaskInfoBean.getTaskTheme().trim() != "") {
                        Map<String, String> map = new HashMap<>();
                        map.put("taskType", changeTaskInfoBean.getTaskType());
                        map.put("taskTitle", changeTaskInfoBean.getTaskTheme());
                        map.put("taskStyle", changeTaskInfoBean.getGlfs());
                        map.put("taskDesc", changeTaskInfoBean.getChangeDescribe());
                        map.put("taskEndTime", changeTaskInfoBean.getNeedDate());
                        map.put("taskPersion", changeTaskInfoBean.getResponsible());
                        map.put("taskState", changeTaskInfoBean.getTaskState());
                        map.put("taskNumber", changeTaskInfoBean.getTaskNumber());
                        map.put("taskActualDate", changeTaskInfoBean.getActualDate());
                        list.add(map);
                    }
                }
                bean.setSwxrw(list);
...
//            }
    ...
    /***
     * 将ECN流程中所有事务性任务转换为ChangeTaskInfoBean对象
     *
     * @param params
     * @return
     * @throws WTException
     */
    public static Collection<ChangeTaskInfoBean> getChangeTaskBeans(WTChangeOrder2 eco) throws WTException {
        Collection<ChangeTaskInfoBean> datasArray = new HashSet<ChangeTaskInfoBean>();
        //获取ECN中所有事务性任务ECA
        QueryResult ecaqr = ChangeHelper2.service.getChangeActivities(eco);
        while (ecaqr.hasMoreElements()) {
            ...
            if (ecaobject instanceof WTChangeActivity2) {
                ...
                //获取ECA任务类型，管理方式对应值
                TransactionTask task = ModifyHelper.service.queryTransactionTask(eco, eca, "");
                String taskType = "";
                String glfs = "";
                if (task != null) {
                ...
                Object needDate = PIAttributeHelper.service.getValue(eca, ChangeConstants.COMPLETION_TIME);//计划完成时间
                String ecaResponsible = EcnChangeTaskService.getAssigneeName(eca);//责任人
                String taskState = EcnChangeTaskService.getEcaState(eca);//状态
        ...
        Set<Persistable> persistables = ModifyHelper.service.queryPersistable(eco, LINKTYPE_3);
        for (Persistable persistable : persistables) {
            if (persistable instanceof TransactionTask) {
                ...
                if (changeActivity2 == null || changeActivity2.isEmpty()) {
                    ...
                    String ecaTaskTheme = task.getChangeTheme() == null ? "" : task.getChangeTheme();//任务主题
                    String changeDescribe = task.getChangeDescribe() == null ? "" : task.getChangeDescribe();//任务描述
                    String needDate = task.getNeedDate() == null ? "" : task.getNeedDate();//计划完成时间
                    String ecaResponsible = task.getResponsible() == null ? "" : task.getResponsible();//责任人
                    changeTaskBean.setTaskType(taskType);
                    changeTaskBean.setTaskTheme(ecaTaskTheme);
                    changeTaskBean.setGlfs(glfs);
                    changeTaskBean.setChangeDescribe(changeDescribe);
