...
public class DocCreateCoverPDF {
     ...
     */
    public <T> File generatePDFs(String name,WTDocument doc) throws WTException, DocumentException, ParseException, MalformedURLException, IOException{
...
//        String realSavePath = new GenerateFold().getFold(savePath);
        ...
        new DocCreateCoverPDF(file).generatePDF(doc);
	 ...
	 * @throws MalformedURLException 
     */
    public <T> File generateYLPDFs(String name,WTDocument doc) throws WTException, DocumentException, ParseException, MalformedURLException, IOException{
...
//        String realSavePath = new GenerateFold().getFold(savePath);
     ...
     * @param file 待生成的文件名
     */
    public DocCreateCoverPDF(File file)
    {
        document.setPageSize(PageSize.A4);// 设置页面大小
        try
        {
    ...
    public DocCreateCoverPDF()
    {
        
    }
    
    public <T> void generatePDF(WTDocument doc) throws WTException, DocumentException, ParseException, MalformedURLException, IOException 
    {
...
//        table1.addCell(createCell("文件受控信息"+image, keyfont, Element.ALIGN_CENTER, 1,false));   
    	...
    	QueryResult qr = WorkflowHelper.service.getWorkItems(doc);
    	 while(qr.hasMoreElements()){
    	        ...
    	        System.out.println("Assessor========================="+users);
    	        if(!users.isEmpty()) {
		    	...
		    	table1.addCell(createCell("", headfont, Element.ALIGN_CENTER,1,false));
		     	table1.addCell(createCell("文件编号："+doc.getNumber(), headfont, Element.ALIGN_LEFT,1,false));      	
		     	table1.addCell(createCell("", headfont, Element.ALIGN_CENTER,1,false));   
		     	table1.addCell(createCell("文件名称："+doc.getName(), headfont, Element.ALIGN_LEFT,1,false));   
		     	table1.addCell(createCell("", headfont, Element.ALIGN_CENTER,1,false));
