...
import static ext.generic.integration.erp.util.SqlHelperUtil.queryisBOM;

public class BomReleaseService {
    private static final Logger logger = LogR.getLogger(BomReleaseService.class.getName());
...
//
//
//                //ecn拆分，增删改为一组，发变更类型为1，立即变；加替代料的为一组，发变更类型为0，用完为止
//                //增删改，变更类型为1
//                Map<WTPart, List<BomCompareBean>> map1 = new HashMap<>();
//                //加替代料，变更类型为0
//                Map<WTPart, List<BomCompareBean>> map2 = new HashMap<>();
                ...
                while (qs.hasMoreElements()) {
                    ...
                    while (qsafter.hasMoreElements()) {
                        ...
                        if (obj instanceof WTPart) {
                            ...
                            //A开头物料的设计视图只发A版,且M视图没有发送过K3（手工发送不做限制）
                                ...
                                if (!mVersion.equals("A")){
                                    ...
                                    if (hVector!=null&&hVector.size()>0){
                                        for (int i = 0; i <hVector.size() ; i++) {
                                            String isSend=getIBAvalue((WTPart)hVector.get(i),"bomReleaseStatus");
...
//            System.out.println("所属公司："+company);
            ...
            for(WTPart childPart:childParts){
                childList.add(childPart.getNumber());
            }

            if (company.equals("FM")){
                //峰米
                List<List<Object>> childFMList= SqlHelperUtil.queryPartsList(childList,"100146");
                String resultStr=BomUtil.compareChildPart(childFMList,childList);
                if (resultStr!=null&&!resultStr.isEmpty()){
                    hashMap.put(part,"K3峰米"+resultStr);
                    System.out.println("K3峰米"+resultStr);
                    return hashMap;
                }
            }
//            else if (company.equals("CINEAPPO")){
//                //中影光峰
//                List<List<Object>> childFMList= SqlHelperUtil.queryPartsList(childList,"100148");
//                String resultStr=BomUtil.compareChildPart(childFMList,childList);
//                if (resultStr!=null&&!resultStr.isEmpty()){
//                    hashMap.put(part,"K3中影光峰"+resultStr);
//                    System.out.println("K3中影光峰"+resultStr);
//                    return hashMap;
//                }
//            }
            else{
                //光峰
                List<List<Object>> childGFList= SqlHelperUtil.queryPartsList(childList,"1");
                String resultStr=BomUtil.compareChildPart(childGFList,childList);
                if (resultStr!=null&&!resultStr.isEmpty()){
                    hashMap.put(part,"K3光峰"+resultStr);
                    System.out.println("K3光峰"+resultStr);
                    return hashMap;
                }
            }
//先发光峰再分配
if ("CINEAPPO".equals(company)){
    company="APPO_CINEAPPO";
}
...
//                }
            ...
            }else if (company.equals("APPO_CINEAPPO")&&isSuccess){
                //峰米
                List<List<Object>> childFMList= SqlHelperUtil.queryPartsList(childList,"100148");
                String resultStr=BomUtil.compareChildPart(childFMList,childList);
                if (resultStr!=null&&!resultStr.isEmpty()){
...
//                System.out.println("APPO_FM 发送 FM BOM");
                String orgid="100148"; //组织(内码)，1光峰，100146峰米,100148中影光峰
                String partNumber=part.getNumber();//编码
                String mversion=part.getVersionIdentifier().getValue();//大版本
                String bomVersion=partNumber+"_"+mversion;
                List<List<Object>> fidList=queryBOMFid(orgid,bomVersion);
                ...
                if (fidList!=null&&fidList.size()>0){
                    //已存在BOM
                    String dataJson2 = KingDeeK3Helper.bomInfoToJson(infos,"CINEAPPO");
                    //发送BOM信息
                    msg = KingDeeK3Helper.saveMaterialBOM(dataJson2);
                }else{
                    //分配
                    String PkIds=viewBOM(bomVersion);
                    String result= fp(PkIds,"100148");
                    System.out.println("分配给中影光峰结果："+result);
                    if (result.contains("true")){
//                        System.out.println("BOM："+bomVersion+"分配给峰米成功");
                    }else{
                        System.out.println("BOM："+bomVersion+"分配给中影光峰失败");
                        hashMap.put(part,"BOM："+bomVersion+"分配给中影光峰失败");
                    }
                }
     ...
     * @throws IOException
     */
    public static WTKeyedHashMap sendGFBOM(WTPart part) {
//        System.out.println("part========ERP===============" + part);
        WTKeyedHashMap hashMap = new WTKeyedHashMap();
        if (part == null) {
            return hashMap;
...
////            System.out.println("所属公司："+company);
            ...
            String dataJson = KingDeeK3Helper.bomInfoToJson(infos,"APPO");
//            System.out.println("结束创建BOM并转为json方法:"+new Date());
//            System.out.println("开始调用金蝶发送BOM接口："+new Date());
            //发送BOM信息
            msg = KingDeeK3Helper.saveMaterialBOM(dataJson);
