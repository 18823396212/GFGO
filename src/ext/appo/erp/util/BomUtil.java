...
import static ext.appo.erp.util.PartUtil.getMaterialGroupData;
import static ext.appo.erp.util.PartUtil.replace;
...
public class BomUtil {
	...
	public static void getAllChildLatestPart(WTPart parent, Set<WTPart> collected) throws WTException {
		QueryResult queryResult = WTPartHelper.service.getUsesWTParts(parent,
	...
	/**
	 * 获取Part所有WTPartUsageLink 即Part的下层子
	 *
	 * @param part
	 * @return
	 * @throws WTException
	...
	public static Map<WTPartMaster, WTPartUsageLink> getAllWTPartUsageLink(WTPart part) throws WTException {
		...
		QueryResult queryresult = PersistenceHelper.manager.navigate(part, WTPartUsageLink.USES_ROLE,
	...
	public static List<BomInfo> createBomBean(WTPart part, Set<WTPart> childParts) throws Exception {
		...
		for (WTPart childPart : childParts) {
			...
			// else {
			// boolean
			// hidePartInStructureValue=childPart.getHidePartInStructure();//是否设计虚拟件
...
//			}
	...
	public static Set<WTPart> getMonolayerPart(WTPart part) {
		...
		// part =
		// MversionControlHelper.getLastObjectAndRelease(part.getMaster());
		if (part != null) {
			QueryResult qr = null;
			try {
				...
				while (qr.hasMoreElements()) {
					...
					// WTPart sunpart = (WTPart)
					// PartUtil.getLastestWTPartByNumber(masterChild.getNumber(),part.getViewName());
					// 不需子件与父件同一视图
		...
		if (part != null) {
			...
			try {
				...
				while (qr.hasMoreElements()) {
					...
					/*
					 * // WTPart sunpart = (WTPart)
					 * PartUtil.getLastestWTPartByNumber(masterChild.getNumber()
					 * ,part.getViewName()); //不需子件与父件同一视图 WTPart sunpart =
					 * (WTPart)
					 * PartUtil.getLastestWTPartByNumber(masterChild.getNumber()
					 * ); // System.out.println("sunpart==="+sunpart);
					 */
					...
					if (ePart != null && mPart != null) {
						if (ePart.getVersionIdentifier().getSeries()
								.greaterThan(mPart.getVersionIdentifier().getSeries())) {
							sunpart = ePart;
						} else {
							sunpart = mPart;
						}
	 ...
	 * 发送BOM时，子项JSON字符串转换
	 */
	public static String createFTreeEntityJson(List<BomInfo> list, String company, K3CloudApiClient client)
			throws Exception {
		...
		for (BomInfo info : list) {
			...
			// queryMaterielFAuxProperty(info.getChildNumber(), client);
			boolean isFIsEnable1 = true;
			...
			// // System.out.println("BOM版本-子件：" + FAuxPropertyId + ";" +
			// // FIsEnable1 + ";" + FUseOrgId);
			// if (company.equals("FM")) {
			// if (FUseOrgId.trim().equals("100146") &&
			// FAuxPropertyId.trim().equals("100001")
			...
			for (WTPartSubstituteLink link : substituteLinks) {
				...
				// FIsEnable1 true为开启，false为未开启
				// FUseOrgId 使用组织 1为光峰
				// List<List<Object>> list3 = queryMaterielFAuxProperty(
				// link.getSubstitutes().getNumber() == null ? "" :
				// link.getSubstitutes().getNumber(), client);
				boolean isFIsEnable2 = true;
				...
				entity2.setFMATERIALIDCHILD(
						link.getSubstitutes().getNumber() == null ? "" : link.getSubstitutes().getNumber());
	 ...
	 * @return
	 */
	public static List<ECNInfoEntity> getECNInfoEntityToJson(Map<WTPart, List<BomCompareBean>> map, String company,
		...
		for (WTPart key : map.keySet()) {
			...
			for (BomCompareBean bean : bomCompareBeans) {
				...
				if (bean.getCompareFlag().equals("A")) {
					...
					// 子项是否开启版本
					boolean isFIsEnable = isFIsEnable(bean.getChildNumber(), company, client);
				...
				if (bean.getCompareFlag().equals("C")) {
					...
					List<List<Object>> queryFENTRYIDList = queryFENTRYID(bean.getBomVersion(), bean.getChildNumber(),
							FUSEORGID);
					String FBomEntryIdStr = "";
					...
					if (bean.getSubstituteNumber() != null && bean.getSubstituteNumber() != "") {
						// 替代料是否开启版本
						boolean isFIsEnable2 = isFIsEnable(bean.getSubstituteNumber(), company, client);
						// 子项是否开启版本
	...
	// 是否开启辅助属性版本
	public static boolean isFIsEnable(String partNumber, String company, K3CloudApiClient client) {
		...
		try {
			list2 = queryMaterielFAuxProperty(partNumber, client);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (list2 != null && list2.size() > 0) {
			...
			for (int y = 0; y < list2.size(); y++) {
				...
				// company 20峰米，02光峰
				if (company.equals("20")) {
					if (FUseOrgId.trim().equals("100146") && FAuxPropertyId.trim().equals("100001")
							&& FIsEnable1.trim().equals("true")) {
						System.out.println("true");
						isFIsEnable1 = true;
						break;
					...
					if (FUseOrgId.trim().equals("1") && FAuxPropertyId.trim().equals("100001")
							&& FIsEnable1.trim().equals("true")) {
						System.out.println("true");
						isFIsEnable1 = true;
						break;
