...
public class ECAWorkflowUtil implements ChangeConstants, ModifyConstants {
    ...
    public void checkAftersIsCheckout(WTObject pbo) throws WTException {
        if (pbo instanceof WTChangeActivity2) {
            List aftersArray = new ArrayList();
            Collection<Changeable2> afters = ModifyUtils.getChangeablesAfter((WTChangeActivity2) pbo);
            for (Changeable2 after : afters) {
                if (!(after instanceof Workable))
                    continue;
                if (PICoreHelper.service.isCheckout((Workable) after)) {
                    aftersArray.add(ModifyUtils.getNumber(after));
                }
            }
            if (aftersArray != null && aftersArray.size() > 0) {
                String numbers = "";
                for (int i = 0; i < aftersArray.size(); i++) {
                    if (i == 0) {
                        numbers = String.valueOf(aftersArray.get(i));
                    } else {
                        numbers += "," + String.valueOf(aftersArray.get(i));
                    }
                }
                throw new WTException("产生对象「" + numbers + "」被检出！");
            }
        }
    }
}
