...
public class ExtEditChangeNoticeFormProcessor extends EditChangeNoticeFormProcessor {
     ...
     * @throws WTException
     */
    public void checkIsStandardparse(NmCommandBean nmcommandBean, Collection<Changeable2> affectedArray) throws WTException {
        // 受影响对象列表不能为空
                        ...
                        if (!partlist.contains(version)) {
                            ...
                            while (eCResult.hasMoreElements()) {
                                ...
                                if (!order.getNumber().startsWith(order2.getNumber())) {
                                    if ((!ChangeUtils.checkState(order2, ChangeConstants.CANCELLED)) && (!ChangeUtils.checkState(order2, ChangeConstants.RESOLVED))) {

                                        throw new WTException("物料：" + part.getNumber() + ",存在没解决的ECN:" + order2.getNumber() + "不能同时提交两个ECN！");
                                    }
                                }
                            }
                        } else {
     ...
     * @throws WTException
     */
    public void checkAffectedItems(WTChangeOrder2 changeOrder2, Collection<Changeable2> affectedArray) throws WTException {
        ...
        StringBuilder errorMsg = new StringBuilder();
        for (Changeable2 changeable2 : affectedArray) {
            if ((!ChangeUtils.checkState((LifeCycleManaged) changeable2, ChangeConstants.ARCHIVED)) && (!ChangeUtils.checkState((LifeCycleManaged) changeable2, ChangeConstants.RELEASED))) {
                if (errorMsg.length() > 0) {
                    errorMsg.append("\n");
                }
