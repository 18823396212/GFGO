...
public class AppoBOMChangeUtil {
	...
	/**
	 * windchill ext.appo.bom.change.AppoBOMChangeUtil -u wcadmin -p pdm -f
	 * changeBOM.xls
	 * 
	 * @param args
	 * @throws IOException
	...
	public static void main(String[] args) throws IOException, WTPropertyVetoException {
		...
		AppoBOMChangeUtil invtBOMChangeUtil = new AppoBOMChangeUtil(filename);
		invtBOMChangeUtil.doOperate();

	}

	public AppoBOMChangeUtil() {
		this.dataVector = null;
		this.filename = "";
	}

	public AppoBOMChangeUtil(String filename) {
	...
	protected Vector verifyBusinessData(Vector dataVector) {
		...
		for (int i = 0; i < dataVector.size(); i++) {
			 ...
			 * StringTokenizer(changeBOMObject.getOccurrence(), ","); while
			 * (st.hasMoreTokens()) { String subStr = st.nextToken().trim();
			 * 
			 * if (oHashMap.containsKey(subStr)) { String oldID =
			 * nHashMap.get(changeBOMObject.getChildnumber());
			 * System.out.println("oldID==" + oldID);
			 * System.out.println("subStr==" + subStr);
			 * System.out.println("changeBOMObject.getId()==" +
			 * changeBOMObject.getId()); oHashMap.put(subStr, oldID == null ?
			 * changeBOMObject.getId() : oldID + "," + changeBOMObject.getId());
			 * feedBackMessage.add("第" + changeBOMObject.getId() + "行" +
			 * changeBOMObject.getQuantity() + ",位号" + subStr + "存在重复值"); } else
