...
public class DataReportICTools1 implements RemoteAccess {

	private static final Logger LOGGER = Logger.getLogger(DataReportICTools1.class);

	public static void main(String[] args) {

		System.out.println("功能：导出IC信息");
		System.out.println("示例：windchill ext.tools.DataReportICTools");
	...
	public static Vector getEpart(Vector partvector, Vector partManufacturvector) throws WTException {
		...
		for (int i = 0; i < partvector.size(); i++) {
			...
			// if (part.getNumber().startsWith("E") &&
			// !part.getState().toString().startsWith("OBSOLESCENCE")) {// 电子料
			if (part.getNumber().startsWith("E")) {// 电子料
				...
				if (nodeHierarchy.contains("E0300000")) {
					...
					object.setECA_before_affectitems(getIBAvalue(part, "sfjckg"));// 是否集成开关
					object.setECA_after_affectitems(getIBAvalue(part, "kgpl"));// 开关频率
					object.setECA_before_affectitemsnumber(getIBAvalue(part, "scdl"));// 输出电流
					object.setECA_before_affectitemsname(getIBAvalue(part, "scdy"));// 输出电压
					object.setECA_before_affectitemsstate(getIBAvalue(part, "fzlx"));// 封装类型
					object.setECA_before_version(getIBAvalue(part, "gzdyfw"));// 工作电压范围
					object.setECA_change_type(getIBAvalue(part, "xplx")); // 芯片类型
					object.setECA_after_affectitemsnumber(getIBAvalue(part, "zydk")); // 增益带宽（放大器）
					object.setECA_after_affectitemsname(getIBAvalue(part, "lx"));// 类型/Type
					object.setECA_after_affectitemsstate(facadeInstance.getLocalizedDisplayNameForClassificationNode(
							part.getState().toString(), CsmConstants.NAMESPACE, new Locale("en")));// 状态
					object.setECA_after_version(getIBAvalue(part, "footprintref")); // 封装名称1/Footprints1
					object.setECA_new_version(getIBAvalue(part, "libraryref"));// 符号名称/Symbols
					object.setECA_new_state(getIBAvalue(part, "librarypath")); // 符号路径/Symbols
																				// Path
					object.setCz(getIBAvalue(part, "footprintpath")); // 封装路径1/footprintpath1
					// object.setRz();
					// object.setEddy()
					// object.setEdgzwd();
