...
public class DataReportDianzuTools implements RemoteAccess {

	private static final Logger LOGGER = Logger.getLogger(DataReportDianzuTools.class);

	public static void main(String[] args) {

		System.out.println("功能：导出电阻信息");
		System.out.println("示例：windchill ext.tools.DataReportDianzuTools");
	...
	public static Vector getEpart(Vector partvector, Vector partManufacturvector) throws WTException {
		...
		for (int i = 0; i < partvector.size(); i++) {
			...
			if (part.getNumber().startsWith("E") && !part.getState().toString().startsWith("OBSOLESCENCE")) {// 电子料
				...
				if (nodeHierarchy.contains("E0100000")) {
					...
					object.setECA_before_affectitems(getIBAvalue(part, "zz"));// 阻值
					object.setECA_after_affectitems(getIBAvalue(part, "zzdw"));// 阻值单位
					object.setECA_before_affectitemsnumber(getIBAvalue(part, "jd"));// 精度
					object.setECA_before_affectitemsname(getIBAvalue(part, "edgl"));// 额定功率
					object.setECA_before_affectitemsstate(getIBAvalue(part, "fzlx"));// 封装类型
					object.setECA_before_version(facadeInstance.getLocalizedDisplayNameForClassificationNode(
