...
public class ExportPartItems2 implements RemoteAccess, Serializable {
	private List<List<String>> datalist;
	public static String filename;
	private HSSFWorkbook hssfWorkbook;

	public ExportPartItems2(List<List<String>> datalist) {
	...
	public void doExport(List<List<String>> datalist) {
		try {
			try {
				...
				// HH:mm:ss");
				String fil_ename = "PLM_K3_Comparison_part_Report" + new Date() + ".xls";
	...
	public static void writeDocToSheet(List<List<String>> datalist, HSSFWorkbook hssfWorkbook, HSSFSheet hssfSheet)
		...
		secondrow.createCell((short) (j++)).setCellValue("物料编号");
		secondrow.createCell((short) (j++)).setCellValue("规格描述");
		secondrow.createCell((short) (j++)).setCellValue("PLM最高版本");
		secondrow.createCell((short) (j++)).setCellValue("K3最高版本");
		secondrow.createCell((short) (j++)).setCellValue("PLM状态");
		secondrow.createCell((short) (j++)).setCellValue("是否一致");

		for (int k = 0; k < datalist.size(); k++) {
			List<String> vector = datalist.get(k);
			HSSFRow row = hssfSheet.createRow((i + k + 1));
			j = 0;
