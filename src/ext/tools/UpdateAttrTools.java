...
public class UpdateAttrTools implements RemoteAccess {
	...
	private static final String configPath_B = WT_CODEBASE + File.separator + "ext" + File.separator + "tools"
			+ File.separator + "excelB.xlsx";
	...
	private final static String attr4Key = "xsxh";// excelB的属性的内部名称（销售型号）
	...
	public static void main(String[] args) {
		...
		System.out.println("功能：修改部件分类");
		System.out.println("示例：windchill ext.tools.UpdateAttrTools");
	...
	public static String execute(String partNumbers) throws Exception {
		...
		for (WTPartMaster partMaster : map.keySet()) {
			...
			for (WTPart part : parts) {
				...
				// 自动合成规格描述,并更新规格描述,add mao
				/**
				 * NumberAttrRule noRule = getNumberAttrRule(part); String ggms
				 * = getMergeValueByRule(part, noRule);
